...
import { vehicleAPI, bookingAPI, getImageUrl } from '../services/api';
  ...
  return (
    <div className="w-full max-w-[1280px] mx-auto px-5 min-h-[calc(100vh-72px)] py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-8 items-start">
        ...
        <div className="animate-fade-in">
          ...
          <div className="mb-6">
            <div className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl h-[360px] md:h-[460px] flex items-center justify-center relative overflow-hidden mb-3">
              {hasImages ? (
                <img src={getImageUrl(vehicle.images[activeImage])} alt={vehicle.title} className="w-full h-full object-cover transition-opacity duration-300" />
              ) : (
          ...
          {/* Title & Location */}
          <h1 className="text-[1.8rem] font-[800] mb-2 text-gray-900 dark:text-white">{vehicle.title}</h1>
          <div className="flex items-center gap-4 flex-wrap mb-6 text-gray-500 dark:text-gray-400 text-[0.9rem]">
            <span className="flex items-center gap-1"><MapPin size={16} /> {vehicle.location}</span>
            <span className="inline-flex items-center gap-1 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">{vehicle.type}</span>
            {vehicle.brand && <span>{vehicle.brand} {vehicle.model}</span>}
            {vehicle.year && <span>• {vehicle.year}</span>}
          </div>

          {/* Specs Grid */}
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-6 mb-6">
            <h3 className="font-[700] mb-4 text-[1rem] text-gray-900 dark:text-white">Specifications</h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {vehicle.specs?.fuel && (
                <div className="flex items-center gap-2.5">
                  <div className="w-10 h-10 rounded-[10px] bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 flex items-center justify-center"><Fuel size={18} className="text-gray-900 dark:text-white" /></div>
                  <div><div className="text-[0.75rem] text-gray-500 dark:text-gray-400">Fuel</div><div className="font-[600] text-[0.9rem] text-gray-900 dark:text-white">{vehicle.specs.fuel}</div></div>
                </div>
              )}
              {vehicle.specs?.seats && (
                <div className="flex items-center gap-2.5">
                  <div className="w-10 h-10 rounded-[10px] bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 flex items-center justify-center"><Users size={18} className="text-gray-900 dark:text-white" /></div>
                  <div><div className="text-[0.75rem] text-gray-500 dark:text-gray-400">Seats</div><div className="font-[600] text-[0.9rem] text-gray-900 dark:text-white">{vehicle.specs.seats}</div></div>
                </div>
              )}
              {vehicle.specs?.transmission && (
                <div className="flex items-center gap-2.5">
                  <div className="w-10 h-10 rounded-[10px] bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 flex items-center justify-center"><Star size={18} className="text-gray-900 dark:text-white" /></div>
                  <div><div className="text-[0.75rem] text-gray-500 dark:text-gray-400">Transmission</div><div className="font-[600] text-[0.9rem] text-gray-900 dark:text-white">{vehicle.specs.transmission}</div></div>
                </div>
              )}
            </div>
            ...
            {vehicle.description && (
              <div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                <h3 className="font-[700] mb-3 text-[1rem] text-gray-900 dark:text-white">Description</h3>
                <p className="text-gray-600 dark:text-gray-300 text-[0.95rem] leading-relaxed whitespace-pre-line">{vehicle.description}</p>
              </div>
            )}
          </div>
          ...
          {vehicle.owner && (
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-6">
              <h3 className="font-[700] mb-4 text-[1rem] text-gray-900 dark:text-white">Owner</h3>
              <div className="flex items-center gap-3.5">
                <div className="w-12 h-12 rounded-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 flex items-center justify-center font-[700] text-gray-900 dark:text-white text-lg">
                  {vehicle.owner.name?.charAt(0).toUpperCase()}
                </div>
                <div>
                  <div className="font-[600] text-gray-900 dark:text-white">{vehicle.owner.name}</div>
                  <div className="text-[0.8rem] text-gray-500 dark:text-gray-400">Member since 2024</div>
                </div>
              </div>
            </div>
          )}
          ...
          {/* Location Map */}
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-6 mt-6">
            <h3 className="font-[700] mb-4 text-[1rem] text-gray-900 dark:text-white flex items-center gap-2">
            ...
            <p className="mt-3 text-[0.85rem] text-gray-500 dark:text-gray-400">
              {vehicle.location}
            </p>
          </div>
        </div>
        ...
        {/* Right: Booking Card */}
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-7 sticky top-[96px] animate-fade-in stagger-2">
          <div className="flex justify-between items-end mb-6">
            <div>
              <div className="text-[2rem] font-[800] text-gray-700 dark:text-gray-300 leading-none mb-1">₹{vehicle.pricePerHour}</div>
              <div className="text-[0.8rem] text-gray-500 dark:text-gray-400">per hour</div>
            </div>
            <div className="text-right">
              <div className="text-[1.3rem] font-[700] text-gray-600 dark:text-gray-300 leading-none mb-1">₹{vehicle.pricePerDay}</div>
              <div className="text-[0.8rem] text-gray-500 dark:text-gray-400">per day</div>
            </div>
          </div>

          {!isAuthenticated ? (
            <div className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-5 text-center mb-4">
              <p className="text-gray-600 dark:text-gray-300 text-[0.9rem] mb-3">Please log in to book this vehicle</p>
              <Link to="/login" className="inline-flex items-center justify-center w-full px-4 py-2.5 text-[0.95rem] font-semibold rounded-lg text-white bg-black dark:bg-white dark:text-black hover:bg-gray-800 transition-all">Login to Book</Link>
            </div>
          ) : user.role !== 'consumer' ? (
            ...
            <form onSubmit={handleBook} className="flex flex-col gap-4">
              ...
              {priceCalc && (
                <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 mt-2">
                  <div className="flex justify-between mb-2 text-[0.85rem] text-gray-500 dark:text-gray-400">
                  ...
                  <div className="border-t border-gray-200 dark:border-gray-700 pt-2 mt-2 flex justify-between font-[700] text-[1.1rem]">
                    <span className="text-gray-900 dark:text-white">Total</span><span className="text-gray-700 dark:text-gray-300">₹{priceCalc.price.toLocaleString()}</span>
                  </div>
                </div>
              )}
          ...
          <div className="flex items-center justify-center gap-2 mt-4 text-gray-500 dark:text-gray-400 text-[0.8rem]">
